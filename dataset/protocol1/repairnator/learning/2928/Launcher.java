            ...
            if (!listOfBuilds.get(status).isEmpty()) {
                ...
                for (BuildToBeInspected buildToBeInspected : listOfBuilds.get(status)) {
                    if (this.config.getLauncherMode() == LauncherMode.REPAIR) {
                        ...
                        Launcher.LOGGER.info("Incriminated project and pair of builds: " + buildToBeInspected.getBuggyBuild().getRepository().getSlug() + ":" + buildToBeInspected.getBuggyBuild().getId() + "" + Utils.COMMA + "" + buildToBeInspected.
getPatchedBuild().getId());
